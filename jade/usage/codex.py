#!/usr/bin/python3
# Vendored from Omarchy (https://github.com/omacom/omarchy), bin/omarchy-agent-usage-codex
# at commit d3cfd53b997f8bdcf776b8db68bf0d735e7a065d. MIT License, Copyright (c)
# David Heinemeier Hansson; see THIRD_PARTY_LICENSES.md. Local changes are marked
# "Jade:" and limited to the cache location, client name and reset-credit count.
...
def cache_root():
  # Jade: scan and probe caches live beside our records, not Omarchy's.
  root = Path(os.environ.get("XDG_CACHE_HOME") or (Path.home() / ".cache")) / "jade-shell" / "usage" / "scan"
...
def fetch_codex_rpc():
  ...
  try:
    rpc_request(proc, 1, "initialize", {"clientInfo": {"name": "jade-shell", "version": "1"}}, timeout=8)
    ...
    # Jade: free full resets the account holds. Counted for display only;
    # redeeming one stays a deliberate act in Codex itself.
    credits = ((limits_msg.get("result") or {}).get("rateLimitResetCredits") or {}).get("credits") or []
    result["resetCredits"] = sum(1 for c in credits if isinstance(c, dict) and c.get("status") == "available")
  except Exception as exc:
