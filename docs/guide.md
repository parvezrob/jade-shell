...
Run it in a terminal on your desktop, as yourself. It checks your system, downloads the latest `.rpm` or `.deb` release (a download that was cut off continues where it stopped the next time), checks it against the release checksums, asks for your password once and installs it with `dnf` or `apt`, waiting if the Software app or automatic updates are busy. Then it runs `jade setup`, and last adds text and QR code reading if you said yes when it asked at the start (Tesseract and ZBar, for the screenshot's **Copy Text** and the QR code reader, and the Wi-Fi QR code; Jade Shell works without them, and running the install command again offers them again). Run again, it only sets up your desktop; `--reinstall` installs the package again. `jade setup`:
