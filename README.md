...
- **Frosted glass** (Settings › Desktop › **Glass**, or the Jade Menu's Toggles): the top bar, every menu, dialogs, notification banners, the volume and brightness pop-ups and the screenshot card blur what is behind them, windows included, tinted by the theme, like the dock. It only works while a surface is on screen, and skips the blur during open and close fades. If Blur my Shell is on, the top bar is left to it.
